  ...
  return (
    <>
      ...
      <main id="top">
        ...
        <section className="section final-cta" id="contact" aria-labelledby="contact-title"><div className="container final-card"><div><p className="section-kicker">Your next step can be small</p><h2 id="contact-title">Have a business in mind?<br />Let&apos;s make the next step simple.</h2></div><div><p>Tell us where you are, what you&apos;re trying to do, or even just what feels confusing. That&apos;s enough to begin.</p><div className="button-row"><a className="button button--cream" href={`mailto:${brand.email}`}>Tell us what you need <Arrow /></a><a className="button button--outline-light" href={brand.whatsapp} target="_blank" rel="noreferrer">Chat on WhatsApp</a></div><p className="contact-note">Prefer a call? <a href={brand.phoneHref}>{brand.phoneDisplay}</a></p></div></div></section>
      </main>
      <footer className="site-footer"><div className="container footer-grid"><div><a className="wordmark wordmark--footer" href="#top"><span className="wordmark-mark">{brand.shortName}</span><span>{brand.name.replace(`${brand.shortName} `, "")}</span></a><p>Digital help for small businesses—made simple.</p></div><div><h2>Explore</h2><a href="#services">Services</a><a href="#work">Concept work</a><a href="#process">How it works</a></div><div><h2>Studio</h2><a href="#about">About</a><a href="#contact">Contact</a><a href="/privacy/">Privacy</a></div><div><h2>Contact</h2><a href={`mailto:${brand.email}`}>{brand.email}</a><a href={brand.phoneHref}>{brand.phoneDisplay}</a><a href={brand.whatsapp} target="_blank" rel="noreferrer">WhatsApp</a></div></div><div className="container footer-bottom"><span>© {new Date().getFullYear()} {brand.name}</span><span>Made for small businesses</span></div></footer>
    </>
  );
}
